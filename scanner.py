...
# =============================================================================
# DELTA AIR LINES FETCHER (Avature, URL pagination)
# =============================================================================
def fetch_delta_jobs(base_url, db, company_id):
    """Fetch Delta Air Lines jobs using Playwright."""
    from playwright.sync_api import sync_playwright
    ...
    with sync_playwright() as p:
        ...
        while page_num < max_pages:
            if offset > 0:
                page_url = f"{base_url}&jobOffset={offset}"
                page.goto(page_url, wait_until='domcontentloaded', timeout=30000)
                page.wait_for_timeout(3000)

            # Delta Avature pattern: /JobDetail/{title}/{id}?jobId={id}
            links = page.locator('a[href*="/careers/JobDetail/"]').all()
            ...
            for link in links:
                try:
                    ...
                    # Skip email share links
                    if href.startswith('mailto') or 'Share' in title:
                        continue

                    job_id_match = re.search(r'/JobDetail/[^/]+/(\d+)', href)
                    if not job_id_match or len(title) < 3:
                        continue
                    ...
                    if job_id not in jobs:
                        full_url = href if href.startswith('http') else f"https://delta.avature.net{href}"
                        restrictions = detect_restrictions(title)
                        jobs[job_id] = {
                            "id": job_id,
                            "title": title,
